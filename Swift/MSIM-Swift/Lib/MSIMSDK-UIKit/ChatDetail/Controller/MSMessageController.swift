...
public class MSMessageController: UITableViewController {
    ...
    private func addNotifications() {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name.init(MSUIKitNotification_MessageUpdate), object: nil, queue: .main) {[weak self] note in
            self?.onMessageUpdate(note: note)
        }
    ...
    private func setupUI() {
        ...
        tableView.register(MSEmotionMessageCell.self, forCellReuseIdentifier: MSMcros.TEmotionMessageCell_ReuseId)
        ;
...
private extension MSMessageController {
    ...
    func onMessageUpdate(note: Notification) {
        guard let message = note.object as? MSIMMessage else {return}
        guard message.partnerID == self.partner_id else {return}
