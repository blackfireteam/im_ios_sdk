...
- (BOOL)addRecord:(MSFileInfo *)info
{
    ...
    NSString *createSQL = [NSString stringWithFormat:@"create table if not exists %@(uuid TEXT,url TEXT,PRIMARY KEY(uuid))",FILE_TABLE_NAME];
    BOOL isOK = [self createTable:FILE_TABLE_NAME withSQL:createSQL];
    if (isOK == NO) {
        NSLog(@"创建表失败****%@",FILE_TABLE_NAME);
        return NO;
    }
    NSString *addSQL = @"REPLACE into %@ (uuid,url) VALUES (?,?)";
    NSString *sqlStr = [NSString stringWithFormat:addSQL,FILE_TABLE_NAME];
    NSArray *addParams = @[info.uuid,XMNoNilString(info.url)];
