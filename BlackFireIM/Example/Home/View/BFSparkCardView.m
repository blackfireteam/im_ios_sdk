...
@class BFCardViewCell;
@protocol BFCardViewCellDelagate <NSObject>
@optional
- (void)cardViewCellDidRemoveFromSuperView:(BFCardViewCell *)cell withDirection:(CardCellSwipeDirection)direction;
- (void)cardViewCellDidMoveFromSuperView:(BFCardViewCell *)cell forMovePoint:(CGPoint)point;
@end

@interface BFCardViewCell()
@property (nonatomic, assign) CGFloat maxAngle;
@property (nonatomic, assign) CGFloat maxRemoveDistance;
@property (nonatomic, assign) CGPoint currentPoint;
@property (nonatomic, assign) NSInteger index;
@property (nonatomic, weak) id<BFCardViewCellDelagate> cell_delegate;

@end

@implementation BFCardViewCell
...
- (void)didPanStateEnded {
    ...
    if (self.currentPoint.x > self.maxRemoveDistance) {
        ...
        [self didCellRemoveFromSuperviewWithDirection:CardCellSwipeDirectionRight];
    ...
    // 左滑移除
        ...
        snapshotView.transform = self.transform;
        [self.superview.superview addSubview:snapshotView];
        [self didCellRemoveFromSuperviewWithDirection:CardCellSwipeDirectionLeft];
...
// 卡片移除处理
- (void)didCellRemoveFromSuperviewWithDirection:(CardCellSwipeDirection)direction {
    self.transform = CGAffineTransformIdentity;
...
- (void)removeFromSuperviewSwipe:(CardCellSwipeDirection)direction {
    switch (direction) {
        case CardCellSwipeDirectionLeft: {
            [self removeFromSuperviewLeft];
        }
            break;
        case CardCellSwipeDirectionRight: {
...
@interface BFSparkCardView()<BFCardViewCellDelagate>
...
/** 当前可视cells */
@property (nonatomic, strong) NSArray<__kindof BFCardViewCell *> *visibleCells;
/** 重用卡片数组  */
@property (nonatomic, strong) NSMutableArray<__kindof BFCardViewCell *> *reusableCells;

@end
@implementation BFSparkCardView
...
- (void)createCardViewCellWithIndex:(NSInteger)index {
    BFCardViewCell *cell = [self.dataSource cardView:self cellForRowAtIndex:index];
    cell.index = index;
...
- (void)updateLayoutVisibleCellsWithAnimated:(BOOL)animated {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        // 获取当前要显示的的cells
        BFCardViewCell *cell = self.visibleCells[i];
...
- (void)updateConstraintsCell:(BFCardViewCell *)cell transform:(CGAffineTransform)transform {
    [UIView animateWithDuration:GX_DefaultDuration animations:^{
        cell.transform = transform;
    } completion:NULL];
}
...
/** 可视cells */
- (NSArray<BFCardViewCell *> *)visibleCells {
...
- (__kindof BFCardViewCell *)dequeueReusableCellWithIdentifier:(NSString *)identifier {
    for (BFCardViewCell *cell in self.reusableCells) {
    ...
    if (self.nib) {
        BFCardViewCell *cell = [[self.nib instantiateWithOwner:nil options:nil] lastObject];
        cell.reuseIdentifier = identifier;
        
        return cell;
    } else if (self.cellClass) { // 注册class
        BFCardViewCell *cell = [[self.cellClass alloc] initWithReuseIdentifier:identifier];
        cell.reuseIdentifier = identifier;
...
- (nullable __kindof BFCardViewCell *)cellForRowAtIndex:(NSInteger)index {
    ...
    if (visibleIndex >= 0 && visibleIndex < self.visibleCells.count) {
        BFCardViewCell *cell = [self.visibleCells objectAtIndex:visibleIndex];
        return cell;
    }
    return nil;
}
...
- (void)removeTopCardViewFromSwipe:(CardCellSwipeDirection)direction {
    ...
    BFCardViewCell *topcell = [self.visibleCells lastObject];
...
- (void)cardViewCellDidMoveFromSuperView:(BFCardViewCell *)cell forMovePoint:(CGPoint)point {
    if ([self.delegate respondsToSelector:@selector(cardView:didMoveCell:forMovePoint:direction:)]) {
        ...
        if (cell.currentPoint.x > cell.maxRemoveDistance) {
            [self.delegate cardView:self didMoveCell:cell forMovePoint:point direction:CardCellSwipeDirectionRight];
        }
        // 左滑中
        else if (cell.currentPoint.x < -cell.maxRemoveDistance) {
            [self.delegate cardView:self didMoveCell:cell forMovePoint:point direction:CardCellSwipeDirectionLeft];
        }
        // 滑动距离不够
        else {
             [self.delegate cardView:self didMoveCell:cell forMovePoint:point direction:CardCellSwipeDirectionNone];
        }
    }
}
